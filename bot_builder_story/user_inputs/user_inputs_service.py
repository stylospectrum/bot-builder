...
from ..proto.bot_builder_nlp.bot_builder_nlp_pb2 import (
...
class UserInputsService:
    def __init__(
        ...
        bot_builder_nlp_service_stub: BotBuilderNlpServiceStubDepend,
    ):
    ...
    def find(self, story_block_id: str):
        return self.session.exec(
            select(UserInput)
            .where(UserInput.story_block_id == story_block_id)
        ...
        for user_input_raw in user_inputs_raw:
            ...
            if user_input_raw["deleted"]:
                ...
                user_inputs.append(
                    UserInput(
                        id=uuid.uuid4(),
                        story_block_id=user_input_raw["story_block_id"],
                        content=user_input_raw["content"],
                    )
                )
                    ...
                    user_inputs=[
                        {
                            ...
                            "story_block_id": str(user_input.story_block_id),
                        }
